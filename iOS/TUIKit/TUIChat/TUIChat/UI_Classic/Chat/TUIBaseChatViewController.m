...
- (void)inputController:(TUIInputController *)inputController didChangeHeight:(CGFloat)height {
    ...
    [UIView animateWithDuration:0.3
        ...
        CGFloat inputH = self.inputController.inputBar.frame.size.height + self.inputController.moreView.frame.size.height + Bottom_SafeHeight;
        CGFloat calHeight;
        if (height > inputH) {
             calHeight = self.view.frame.size.height - height - [self topMarginByCustomView] - self.bottomContainerView.mm_h;

        }
        else{
            calHeight = self.view.frame.size.height - inputH - [self topMarginByCustomView] - self.bottomContainerView.mm_h;
